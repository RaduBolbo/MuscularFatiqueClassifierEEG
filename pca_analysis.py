...
for fold_idx, val_subjects in enumerate(val_subjects_cross_validation):
    ...
    for filename in tqdm(os.listdir(data_directory)):
        if filename.endswith('.pkl'):
            ...
            if flag:
                ...
                feature_vector = load_features_from_file(file_path)
    ...
    #print('dd', len(principal_components[0]))

    principal_components_0 = np.ndarray((1,2))
    principal_components_1 = np.ndarray((1,2))
    for i in range(principal_components.shape[0]):
        if y_train[i] == 0:
            #print('a')
            #print(principal_components_0, principal_components[i])
            principal_components_0 = np.concatenate((principal_components_0, principal_components[i].reshape(1, -1)), axis=0)
        elif y_train[i] == 1:
            #print(principal_components_1, principal_components[i])
            principal_components_1 = np.concatenate((principal_components_1, principal_components[i].reshape(1, -1)), axis=0)

    

    # The result
    #print(principal_components)
    #print(principal_components_0)
    plt.figure(figsize=(8, 6))
    #plt.scatter(principal_components[:, 0], principal_components[:, 1], c='black')
    plt.scatter(principal_components_0[:, 0], principal_components_0[:, 1], c='red')
    plt.scatter(principal_components_1[:, 0], principal_components_1[:, 1], c='blue')
